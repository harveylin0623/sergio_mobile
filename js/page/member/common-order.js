  ...
  new Vue({
    ...
    methods: {
      ...
      async cancelOrder() {
        ...
        this.isLoading = true
        let { orderNumber, isPaid } = this.tempOrderInfo
        ...
        if (this.tipInfo.status) {
          ...
          if (isPaid) {
            location.href = `${this.pageUrl.orderPayRefund}?orderNumber=${orderNumber}`
            return
          }
        } else {
          $('#tipPopup').modal('show')
      ...
      async buyAgain({ orderNumber }) {
        this.isLoading = true
        let response = await orderApi.order_buy_again({ url: apiUrl.order_buy_again, data: { order_num: orderNumber } })
        this.tipInfo.status = response.status === 1
        this.tipInfo.message = response.message
        if (this.tipInfo.status) this.myHeader.updateCartCount(response.cartTotal)
        $('#tipPopup').modal('show')
        this.isLoading = false
      }
    },
